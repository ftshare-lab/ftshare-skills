...
| 外部接口 | /gateway/api/v1/market/data/daec/history/ohlcs                                                                    |
| 请求方式 | GET                                                                                                            |
| 适用场景 | 获取 A 股指定股票在指定日期区间、周期的 K 线（开高低收、成交量、成交额），支持日/周/月线与前/后复权 |

> 已从 v2（`/api/v2/stocks/:stock/ohlcs`）迁移至 daec 统一标的接口（`daec/history/ohlcs?symbol=`）。**对外参数契约有变**：由 v2 的 `--span/--limit/--until_ts_ms` 改为 `--since/--until`（YYYYMMDD 日期区间）+ `--interval` + `--adjust`。注意：daec **不支持年线（YEAR1）**，响应**不再含 MA5/MA10/MA20 与 prev_close**。

## 请求参数

说明：`stock` 为路径参数（必填），`since` 必填，`until`/`interval`/`adjust` 可选。

| 参数名   | 类型   | 是否必填 | 描述                  | 取值示例    | 备注                                                  |
|----------|--------|----------|-----------------------|-------------|-------------------------------------------------------|
| stock    | string | 是       | 股票标的键（带市场后缀） | 600000.XSHG | 沪 .XSHG、深 .SZ、北交所 .BJ                           |
...
# 周线
python <RUN_PY> stock-ohlcs --stock 600000.XSHG --since 20240101 --until 20260628 --interval Week

# 前复权月线（until 默认今天）
python <RUN_PY> stock-ohlcs --stock 600000.XSHG --since 20230101 --interval Month --adjust Forward
```
...
返回 K 线列表，包装为 `{"ohlcs": [...]}`（daec 返回裸数组，脚本统一包装）：
